...
const ClientsEdit = () => {
    ...
    const save = async (event) => {
        ...
        var {name, rol} = document.forms[0];
        var errors = "";
        errors += name.value.length < 0 ? "Debe colocar el nombre del cliente.\n": "";
        errors += rol.value.length < 0 ? "Debe colocar el rol del cliente.\n": "";
        if(errors.length > 0){
            ...
            const requestOptions = {
                ...
                body: JSON.stringify({ "operatorId": config.operatorId, "id": clientData.Id, "name": name.value,"rol": rol.value, "active": clientData.active})
              }
                            ...
                            <div className="row">
                                <div className="col-12">
                                    <div className="form-group">
                                        <label htmlFor="name" className="control-label">Nombre del Cliente</label>
                                        <input type="text" name="name" id="name" className="form-control" defaultValue={clientData.name} required />
                                    </div>
                                </div>
                                <div className="col-4">
                                    <div className="form-group">
                                        <label htmlFor="rol" className="control-label">Rol del Cliente (Rut)</label>
